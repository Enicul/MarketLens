...
def analyze_stock(ticker: str, intents: list[str] = ["news"]) -> str:
    """Complete stock analysis with research - analyst + researcher pipeline.
    ...
    try:
        ...
        # Execute analyst + researcher pipeline
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        
        async def run_full_analysis():
            # Step 1: Analyst data collection
            analyst_result = await analyze_for_manager(ticker.upper(), enabled_intents)
            
            # Step 2: Researcher analysis
            research_result = await research_for_manager(ticker.upper(), analyst_result)
            
            return research_result
        
        result = loop.run_until_complete(run_full_analysis())
        loop.close()
...
def build_main_agent(config=None):
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", 
         ...
         f"Goal: Understand user needs -> Execute complete analysis -> Deliver professional insights\n"
         f"Guidelines:\n"
         f"- analyze_stock now includes full pipeline: data collection + bullish/bearish research + investment recommendation\n"
         f"- Intelligently select analysis combinations based on user needs:\n"
         ...
         f"- File operations via read_file/write_file\n"
         f"- {analysis_desc}. Only use enabled analysis types\n"
